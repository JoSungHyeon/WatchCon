...
function calculateRelevance(
  ...
  const lengthNormalizationFactor = Math.log(
    content.length + 1,
  );
    ...
    chunk
      .map((doc) => {
        ...
        const cleanedContent =
          memoizedCleanMdxContent(content);
            ...
            (word) =>
              ...
              (doc.description &&
                doc.description
                  ...
                  .includes(word)) ||
              (doc.snippet &&
                doc.snippet.toLowerCase().includes(word)),
          )
        );
      })
