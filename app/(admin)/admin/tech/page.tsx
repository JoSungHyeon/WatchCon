...
import styles from '../../../components/admin/tech/style/Tech.module.css';
...
const TechList = dynamic(
  () => import('../../../components/admin/tech/TechList'),
  { ssr: false },
);
const ReplyForm = dynamic(
  () => import('@/app/components/admin/tech/ReplyForm'),
  { ssr: false },
);
const DetailForm = dynamic(
  () => import('@/app/components/admin/tech/DetailForm'),
  { ssr: false },
);
...
const TechPage = () => {
  ...
  useEffect(() => {
    if (data?.list) {
      const filtered = data.list.filter(
        (item: TechItem) => {
          if (item.request_type !== 2) return false;
  ...
  const handleDetailChange = (item: TechItem) => {
    setSelectedItem(item);
    setShowDetail(true);
    setIsReply(false);
  };
  ...
  const handleSearch = () => {
    ...
    const filtered = data.list.filter((item: TechItem) => {
      if (item.request_type !== 2) return false;
